...
class TrainLoop:
    ...
    def run_loop(self):
        ...
        for index_t, dataset_name_t in enumerate(self.args.dataset.split('*')):
            ...
            for index_mask, batch2 in enumerate(self.test_data[index_t]):
                ...
                error_before_scaler += mean_squared_error(samples.reshape(-1,1).detach().cpu().numpy(),target.reshape(-1,1).detach().cpu().numpy())/batch2[0].shape[0]
                error_after_scaler += mean_squared_error(self.args.scaler[dataset_name_t].inverse_transform(samples.reshape(-1,1).detach().cpu().numpy()),self.args.scaler[dataset_name_t].inverse_transform(target.reshape(-1,1).detach().cpu().numpy()))/batch2[0].shape[0]
    ...
    def short_long_temporal_masking(self, x, mask_ratio):
        ...
        mask = rearrange(
            mask,
            'b c t h w (tp ph pw) -> b c (t tp) (h ph) (w pw)',
            tp=self.args.t_patch_size, ph=self.args.patch_size, pw=self.args.patch_size
        )
        return mask.float()
